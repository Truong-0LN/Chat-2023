...
const Chat = () => {
    ...
    if (token) {
      ...
      socket.on('pushRoom', async data => {
        ...
        setRooms(rooms => {
          let ind = -1;
          rooms.map((i, index) => {
            if (i?._id.toString() === data?._id.toString()) {
              ind = index;
            }
          });
          if (ind > -1) {
      ...
      // });
      axios
        .get(`${ENDPOINT}/room/get_list?lang=vi`, configs)
        .then(res => {
  ...
  useEffect(() => {
    // auto gọi các đoạn chat của phòng đó
    if (room !== '' && token) {
      axios
        .get(`${ENDPOINT}/message/get_message/${room}`, configs)
        .then(res => {
  ...
  const login = async () => {
    ...
    axios
      .post(`${ENDPOINT}/${typeUser}/auth/login`, data)
      .then(res => {
  ...
  const getUserByToken = () => {
    axios
      .get(`${ENDPOINT}/${typeUser}/profile/get_info_by_token`, configs)
      .then(res => {
        setUser(res?.data);
      })
      .catch(err => {
  ...
  return (
    <div className="chat section-margin section-padding">
      {token && (
        <div className="chat-list__room">
          <h3>Danh sách phòng trò chuyện</h3>
      ...
      <div className={`${'chat__login'}`}>
        ...
        <input
          ...
          placeholder="Nhập id client"
        />
        <button onClick={onCall}>Call</button>
        <button disabled={!(phone !== '' && password !== '')} onClick={login}>
          Đăng nhập
        </button>
        {user ? (
